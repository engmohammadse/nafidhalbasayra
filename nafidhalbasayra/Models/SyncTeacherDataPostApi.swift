...
class SyncTeacherDataPostApi {
   ...
   // إنشاء URLSession مخصص مع إعدادات محسّنة للإنترنت الضعيف جداً
   private lazy var customURLSession: URLSession = {
       let configuration = URLSessionConfiguration.default
       
       // زيادة المهلة الزمنية للاتصال إلى 10 دقائق
       configuration.timeoutIntervalForRequest = 600.0
       
       // زيادة المهلة الزمنية للموارد إلى ساعة كاملة
       configuration.timeoutIntervalForResource = 3600.0
       
       // السماح بالاتصال عبر الشبكات المحدودة
       configuration.allowsCellularAccess = true
       
       // تمكين انتظار الشبكة المناسبة
       ...
       // إعدادات إضافية للإنترنت البطيء جداً
   ...
   private func processAndSendData(viewModel: TeacherDataViewModel,
       ...
       // إعدادات timeout مخصصة لهذا الطلب - ساعة كاملة
       request.timeoutInterval = 3600.0  // ساعة كاملة للطلب الواحد
       ...
       // الحفاظ على حجم الضغط 900KB كما هو مطلوب
       ...
       // استخدام الـ URLSession المخصص بدلاً من الافتراضي
       let task = customURLSession.dataTask(with: request) { data, response, error in
           
           // تحسين معالجة الأخطاء لتشمل أخطاء الشبكة
           if let error = error {
               DispatchQueue.main.async {
                   ...
                   // حفظ رسالة خطأ مخصصة للإنترنت الضعيف
                   if error.localizedDescription.contains("timeout") || error.localizedDescription.contains("network") {
                       UserDefaults.standard.set("ضعف في الاتصال بالإنترنت، يرجى المحاولة مرة أخرى", forKey: "serverErrorMessage")
                   } else {
